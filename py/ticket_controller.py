...
async def gen_basic_manual(user: User):
    ...
    with open('../cfg/usr_man.txt', 'r', encoding='utf-8') as f:
        for line in f:
            cd.append(Module.Section(line))
...
async def manual(msg: Message, txt: str):
    if txt == '':
        ...
        try:
            with open('../cfg/man/' + txt + '.man') as f:
                cm = CardMessage()
                cd = Card()
                cd.append(Module.Header('帮助手册'))
                for line in f:
...
async def set_role(b: Bot, event: Event, tag: str, role: str):
    ...
    for r in roles:
        if r.id == int(role):
            ...
            return
    await cnl.send("没有找到 " + role + " 角色, 请尝试重新拉去角色列表")
...
async def create_ticket(b: Bot, event: Event) -> Union[str, None]:
    ...
    for c in cate:
        if c.name == 'ticket':
            ...
            if res is not None:
                cnl = await b.fetch_public_channel(event.body['target_id'])
                await cnl.send("您的Ticket已经创建，请查看(chn)" + res + "(chn)", type=MessageTypes.KMD, temp_target_id=event.body['user_id'])
            return
    # 没有Ticket分类就不发了，并且报个临时错误
    cnl = await b.fetch_public_channel(event.body['target_id'])
    await cnl.send("Ticket创建失败，请查看是否存在ticket分类。 *Kook目前不允许机器人创建分类，请手动进行*", type=MessageTypes.KMD, temp_target_id=event.body['user_id'])
